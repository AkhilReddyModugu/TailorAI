"""Manual test script for app.prompts.step4_bullet_rewrite.

Uses the outputs of steps 1-3 as input, so it also exercises the full
parse -> analyze -> gap-detect -> rewrite chain. Runs three sample
resume/JD pairs chosen to land in different agent_strategy buckets
(light_tweaks, targeted_rewrite/aggressive_rewrite, skip_mismatch).

Run with: python test_step4_bullet_rewrite.py
"""
...
async def run_case(label: str, resume_text: str, jd_text: str) -> None:
    ...
    if not rewritten:
        print("\n(no bullets rewritten)")
        return

    print("\n--- Side-by-side rewrite ---")
    for i, rb in enumerate(rewritten, 1):
        print(f"\n[{i}] ORIGINAL : {rb.original}")
        print(f"    REWRITTEN: {rb.rewritten}")
        print(f"    REASON   : {rb.reason}")

    print("\n--- Raw JSON ---")
    print(json.dumps([rb.model_dump() for rb in rewritten], indent=2))
